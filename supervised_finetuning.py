...
def main():
    ...
    def preprocess_function_pilot(examples):
        all_instructions, all_inputs, all_outputs, all_answers = examples["instruction"], examples["input"], examples["output"], examples["answer"]
        input_ids_list = []
        attention_mask_list = []
        targets_list = []
        for instruction, input, output, answer in zip(all_instructions, all_inputs, all_outputs, all_answers):
            input_ids = []
            labels = []

            single_example = {"instruction": instruction, "input": input, "output": output, "answer": answer}
            messages = [single_example.get("instruction")+single_example.get("input"), single_example.get("output")]
            system_prompt = prompt_template.system_prompt
            dialog = prompt_template.get_dialog([messages], system_prompt=system_prompt)
